...
def generate_neural_gaussians(viewpoint_camera, pc : GaussianModel, visible_mask=None, is_training=False):
    ...
    ## TODO 
    # ob_view => view_embedding 
    # appearance => appearance_embedding
    feat_dir = pc.encoder_dir(ob_view)

    cat_local_view = torch.cat([feat, feat_dir, ob_dist], dim=1) # [N, c+3+1]
    cat_local_view_wodist = torch.cat([feat, feat_dir], dim=1) # [N, c+3]
    if pc.appearance_dim > 0:
        camera_indicies = torch.ones_like(cat_local_view[:,0], dtype=torch.long, device=ob_dist.device) * viewpoint_camera.uid
        # camera_indicies = torch.ones_like(cat_local_view[:,0], dtype=torch.long, device=ob_dist.device) * 10
        appearance = pc.get_appearance(camera_indicies) # [N, appearance_dim]
