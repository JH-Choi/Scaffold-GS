...
class GaussianModel:
    ...
    def __init__(self, 
        ...
        self.encoding_xyz = GridEncoder(input_dim=input_dim, num_levels=num_levels, level_dim=level_dim,\
                            base_resolution=base_resolution, log2_hashmap_size=log2_hashmap_size, \
                            desired_resolution=desired_resolution, gridtype='hash', \
                            align_corners=align_corners, interpolation=interpolation).cuda()
        ...
        self.mlp_opacity = nn.Sequential(
            nn.Linear(feat_dim*2+3+self.opacity_dist_dim, feat_dim),
            nn.ReLU(True),
        ...
        self.mlp_cov = nn.Sequential(
            nn.Linear(feat_dim*2+3+self.cov_dist_dim, feat_dim),
            nn.ReLU(True),
        ...
        self.mlp_color = nn.Sequential(
            nn.Linear(feat_dim*2+3+self.color_dist_dim+self.appearance_dim, feat_dim),
            nn.ReLU(True),
        ...
        self.mlp_grid = nn.Sequential(
            nn.Linear(self.encoding_xyz.output_dim, feat_dim),
            nn.ReLU(True),
