...
class GaussianModel:
    ...
    def __init__(self, 
        ...
        bound = 1 # bound 16? TODO 
        self.encoder_dir, self.in_dim_dir = get_encoder('sh')
        # self.encoder, self.in_dim = get_encoder("hashgrid", num_levels=16, level_dim=2, \
        ...
        self.mlp_opacity = nn.Sequential(
            nn.Linear(feat_dim + self.in_dim_dir + self.opacity_dist_dim, feat_dim),
            nn.ReLU(True),
        ...
        self.mlp_cov = nn.Sequential(
            nn.Linear(feat_dim + self.in_dim_dir + self.cov_dist_dim, feat_dim),
            nn.ReLU(True),
            nn.Linear(feat_dim, 7*self.n_offsets),
        ).cuda()

        self.color_dist_dim = 1 if self.add_color_dist else 0
        # Add one more layer for self.mlp_color
        self.mlp_color = nn.Sequential(
            nn.Linear(feat_dim + self.in_dim_dir + self.color_dist_dim + self.appearance_dim, feat_dim),
            nn.ReLU(True),
    ...
    def calc_interp_feat(self, x):
        ...
        features = self.encoder(x)  # [N, 4*12]
        return features
    ...
    def training_setup(self, training_args):
        ...
        if self.use_feat_bank:
            l = [
                ...
                {'params': self.encoder_dir.parameters(), 'lr': training_args.encoder_dir_lr_init, "name": "encoder_dir"},
        ...
        self.encoder_dir_scheduler_args = get_expon_lr_func(lr_init=training_args.encoder_dir_lr_init, 
                                                        lr_final=training_args.encoder_dir_lr_final,
                                                        lr_delay_mult=training_args.encoder_dir_lr_delay_mult,
                                                        max_steps=training_args.encoder_dir_lr_max_steps)
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            ...
            if param_group["name"] == "mlp_color":
                ...
                param_group['lr'] = lr
            if param_group["name"] == "encoder_dir":
                lr = self.encoder_dir_scheduler_args(iteration)
    ...
    def save_mlp_checkpoints(self, path, mode = 'unite'):#split or unite
        ...
        if mode == 'split':
            self.mlp_opacity.eval()
            opacity_mlp = torch.jit.trace(self.mlp_opacity, (torch.rand(1, self.feat_dim+self.in_dim_dir+self.opacity_dist_dim).cuda()))
            opacity_mlp.save(os.path.join(path, 'opacity_mlp.pt'))
            self.mlp_opacity.train()

            self.mlp_cov.eval()
            cov_mlp = torch.jit.trace(self.mlp_cov, (torch.rand(1, self.feat_dim+self.in_dim_dir+self.cov_dist_dim).cuda()))
            cov_mlp.save(os.path.join(path, 'cov_mlp.pt'))
            self.mlp_cov.train()

            self.mlp_color.eval()
            color_mlp = torch.jit.trace(self.mlp_color, (torch.rand(1, self.feat_dim+self.in_dim_dir+self.color_dist_dim+self.appearance_dim).cuda()))
    ...
    def load_mlp_checkpoints(self, path, mode = 'unite'):#split or unite
        if mode == 'split':
            ...
            self.encoder_dir.load_state_dict(checkpoint['encoder_dir'])
        else:
            raise NotImplementedError
